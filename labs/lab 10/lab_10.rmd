...
kabbage_plus_all <- all_counties %>% 
  inner_join(kabbage_counties, by=c('project_county_name')) %>% 
  rename(total_loans=total.x, kabbage_loans=total.y, match_county=project_county_name, state=project_state.x) %>% mutate(percentage_kabbage=(kabbage_loans/total_loans)*100)
