...
state_pop_and_ppp <- virginia_ppp %>%
 ...
 county_pop_and_ppp <- va_county_pop %>% 
     left_join(virginia_ppp, by="project_county_name") %>%
   drop_na(amount) %>% 
   select(estimate,geometry) %>% 
   summarise(count=n())
...
ccbps <- write_csv("C:\\Users\\kates\\Downloads\\cbp19co.zip")

county_religion <- fips_and_naics %>% 
  filter(title == "Religious Organizations") 
