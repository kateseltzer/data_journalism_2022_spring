...
**Answer**
dataframe with 152,584 tweets, columns include things like username, content, date it was created, etc.
```{r}
...
**Answer**
returns unique iterations of words 
```{r}
...
**Answer**
data frame of unique words (all the same case)
```{r}
...
**Answer**
not particularly, includes of a lot of prepositions, articles, etc
...
**Answer**
yes, more specific words like covid, health, relief, vaccine
```{r}
unique_words %>%
...
**Answer**
two data frames with top 10 words by count and percengage, vaccine is mentioned much more frequently in 2021 
```{r}
unique_words_2020 %>%
  anti_join(stop_words) %>%
  group_by(word) %>%
...
**Task** Run the following code and describe the results.
**Answer**
shows top 10 bigrams (includes things like health care, public health, coronavirus pandemic)
```{r}
covid_tweets %>%
  filter(created < '2021-01-01') %>%
...
**Task** Run the following code and describe the results.
**Answer**
similar data frame, really shows how prevalant vaccine became in 2021
```{r}
covid_tweets %>%
  filter(created >= '2021-01-01') %>%
  unnest_tokens(bigram, content, token = "ngrams", n = 2) %>%
...
**Task** Run the following code and describe the results.
**Answer**
fewer Republican tweets in general, greater emphasis on health and health care on Democratic side, greater emphasis on "american people" on the Republican side
...
**Task** Run the following code and describe the results.
**Answer**
2021: "illegal immigrants", switch to biden from trump
```{r}
covid_tweets %>%
...
**Answer**
shows list of most common words and whethere they're positive or negative. positive is positive (but if it's COVID positive could be a problem)
```{r}
