...
**Answer**
Save the data as a new file called "maryland_ppp_top_counties"
Sort the data by project_county_name, count the number lof loans by county, and arrange from the greatest number of loans to the least. Show the top ten counties
```{r}
...
**Answer**
re arranged counties in order of number of loans
```{r}
maryland_ppp_top_counties %>%
  ggplot() +
...
**Task** Run the following code. Explain what you see after running the code in the space below. What has changed from the last codeblock?
**Answer**
counties arranged from least to greatest number of loans, rather than in alphabetical order
```{r}
maryland_ppp_top_counties %>%
  ggplot() +
  geom_bar(aes(x=reorder(project_county_name,total_loans), weight=total_loans)) +
...
**Task** Run the following code. Explain what you see after running the code in the space below. 
**Answer**
dataframe with two columns, date approved and total loans
```{r}
...
**Task** Run the following code. Explain what you see after running the code in the space below. 
**Answer**
line graph, early spike and then long flat line, then small spike
```{r}
ppp_maryland_loans_by_date %>%
  ggplot() + 
...
**Task** Run the following code. Explain what you see after running the code in the space below. 
**Answer**
same, but one tick mark per month on x axis
```{r}
ppp_maryland_loans_by_date %>%
  ggplot() + 
  geom_line(aes(x=date_approved, y=total_loans)) + 
