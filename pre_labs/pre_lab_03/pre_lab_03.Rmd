...
**Answer**
Double (number); character; double
```{r}
...
**Answer**:
135073 - yes - good that there's not a forgiveness date without a forgiveness amount, telling us that the data says what we think it will
```{r}
ppp_maryland_loans %>%
...
**Answer**
largest: $10,000,000 smallest: $6.The larger number makes sense because that was the max PPP loan amount, but I'm suspicious that $6 is an instance of bad data because I don't know why any small business would take out a loan for $6.
```{r}
...
**Answer**
weirdly yes, they're consistent across amount/initial approval amount/current approval
```{r}
...
**Answer**
some names are duplicated, multiple spellings/case of Arnold, something weird going on with id column
```{r}
arnold_md_loans
```
...
**Answer**
problem with summarise, invalid type. I think that the amount column is registering as a character data type, and R needs it to be a number to calculate sum(amount)
```{r}
# cleaning function
