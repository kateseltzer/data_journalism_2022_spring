...
**Answer**
This table describes in very general terms different industries by their two-digit NAICS codes. Some sectors are categorized within a range (for example, 31-33 are all classified within manufacturing).
...
**Answer**
Shows the source HTML code behind the webpage, including tables that include rows and cells.
...
**Answer**
naics_industries "List of 2". Not a dataframe, but shows the source code of the bage.
```{r}
# read in the html
...
**Answer**
A bit of html, but it's kind of gibberish
...
**Answer**
list of six data frames, each table converted to dataframe
```{r}
...
**Task** Follow the directions in the previous paragraph. Briefly describe what you see there.
**Answer**
Explains what each of the tibbles are and gives example of what's in them.
...
**Task** Follow the directions in the previous paragraph. Briefly describe what you see there.
**Answer**
Shows two columns, sector and description
### Task 16: Load image
...
**Answer**
the second table (only) is displayed as a dataframe 
```{r}
...
**Answer**
column names are lowercase, 21st row is gone
```{r}
# Read in all html from table, store all tables on page as nested list of dataframes.
naics_industry <- naics_url %>%
  read_html() %>%
...
**Answer**
Dataframe with 20 rows, 2 columns: sector and description
```{r}
...
**Task** Visit the web page linked above. Briefly describe what you see there.
**Answer**
Describes the "Utilities" sector, not all of the sectors together (each one is on a different page)
...
**Answer**
html that exists at the specific path
```{r}
...
**Answer**
dataframe with 2 columns, 1 row
```{r}
...
**Answer**
list of industry groups after [1]
```{r}
...
**Answer**
creates a value called list_of_sectors that contains 10 industry groups
```{r}
...
**Answer**
Error in sector_list: sector_list not found
```{r, error=TRUE}
# Define list of sectors
list_of_sectors <- c("Agriculture, Forestry, Fishing and Hunting", "Mining, Quarrying, and Oil and Gas Extraction", "Utilities", "Construction", "Manufacturing",
"Wholesale Trade", "Retail Trade", "Transportation and Warehousing", "Information", "Finance and Insurance")
...
**Task** Run the following code to use a for loop to print list of sectors. Briefly describe the output that appears below the codeblock.
**Answer**
list of sectors (do you always need to define the list? that seems like the same amount of code as before)
```{r}
# Define list of sectors
list_of_sectors <- c("Agriculture, Forestry, Fishing and Hunting", "Mining, Quarrying, and Oil and Gas Extraction", "Utilities", "Construction", "Manufacturing",
"Wholesale Trade", "Retail Trade", "Transportation and Warehousing", "Information", "Finance and Insurance")
...
**Task** Run the following code to use a for loop to print list of sectors. Briefly describe the output that appears below the codeblock.
**Answer**
error in print(sector_name): object 'sector_name_ not found
```{r, error=TRUE}
# Define list of sectors
list_of_sectors <- c("Agriculture, Forestry, Fishing and Hunting", "Mining, Quarrying, and Oil and Gas Extraction", "Utilities", "Construction", "Manufacturing",
"Wholesale Trade", "Retail Trade", "Transportation and Warehousing", "Information", "Finance and Insurance")
...
**Answer**
tibble with 20 rows, sector and description columns
...
**Task** Run the following code to load table from last chapter. Briefly describe the output that appears below the codeblock.
**Answer**
20x2 tibble with sector and description
```{r}
# Define url of page we want to scrape
...
**Answer**
tibble with sector, description, sector url
```{r}
...
**Answer**
same but new column with sector xpath id
```{r}
...
**Task** Visit the web page linked above. Briefly describe what you see there.
**Answer**
shows employment and unemployment for those industries
### Task 37: Load image
**Task** Run the following code to display an image showing what you should see on the web page.
