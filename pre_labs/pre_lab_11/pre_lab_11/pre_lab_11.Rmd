...
**Answer**
list of files, including shapefile
```{r, echo=FALSE}
...
**Answer**
data frame with hospitals, includes geometry column
```{r}
...
**Answer**
table (sort of) with MD hospitals, including their Geometry
```{r}
...
**Task** Run the following code. Describe the output in the space below.
**Answer**
shows that there are 50 acute care hospitals in Maryland (dataframe with three columns)
```{r}
md_hospitals %>%
  group_by(TYPE) %>%
  summarise(
...
**Task** Run the following code. Describe the output in the space below.
**Answer**
dataframe with only general acute care Maryland hospitals
```{r}
...
**Task** Run the following code. Describe the output in the space below.
**Answer**
scatter plot with points plotted at longitude and latitude, cluster at 39-77 ish. 
```{r}
md_hospitals %>%
  ggplot() + 
  geom_sf() + 
...
**Task** Run the following code. Describe the output in the space below.
**Answer**
dataframe of counties, info about each county
```{r}
...
**Task** Run the following code. Describe the output in the space below.
**Answer**
dataframe of Maryland counties
```{r}
...
**Task** Run the following code. Describe the output in the space below.
**Answer**
map of Maryland divided by county (includes water)
```{r}
md_counties %>%
  ggplot() + 
  geom_sf() + 
...
**Task** Run the following code. Describe the output in the space below.
**Answer**
MD hospitals plotted on top of MD counties
```{r}
...
**Task** Run the following code. Describe the output in the space below.
**Answer**
map with MD counties, hospitals - background of counties is white, hospital points color coded by number of beds with slightly larger points
```{r}
...
**Answer**
all libraries + census key loaded
```{r}
library(tidyverse)
library(sf)
library(janitor)
...
**Task** Run the following code. Describe the output in the space below.
**Answer**
data frame with Maryland acute care hospitals, based on shapefile
```{r}
...
**Task** Run the following code. Describe the output in the space below.
**Answer**
data frame with Maryland beds by county, summarized in order of counties with the greatest number of beds
```{r}
...
**Task** Run the following code. Describe the output in the space below.
**Answer**
dataframe with Maryland population by county
```{r}
...
**Task** Run the following code. Describe the output in the space below.
**Answer**
dataframe with county, population, and geometry columns
```{r}
...
**Task** Run the following code. Describe the output in the space below.
**Answer**
table with county and total beds
```{r}
...
**Task** Run the following code. Describe the output in the space below.
**Answer**
match the dataframes, columns are county, population, and geometry
```{r}
...
**Task** Run the following code. Describe the output in the space below.
**Answer**
joined data frames with maryland beds by county and maryland county pop
```{r}
...
**Task** Run the following code. Describe the output in the space below.
**Answer**
data frame with county, pop, total beds, geometry, and a new column that calculates beds per 100,000x
```{r}
md_beds_per_100k <- md_county_population %>%
  left_join(md_beds_by_county) %>%
  mutate(total_beds = replace_na(total_beds,0)) %>%
...
**Task** Run the following code. Describe the output in the space below.
**Answer**
map with blue color scheme that shows hospital beds per 100,000 by county
```{r}
ggplot() + 
...
**Task** Run the following code. Describe the output in the space below.
**Answer**
same but with magma color scheme, shows a little more contrast
```{r}
ggplot() + 
  geom_sf(data=md_beds_per_100k, aes(fill=beds_per_100k)) + 
  theme_minimal() +
...
**Answer**
same map but with log color scheme to show more contrast 
```{r}
ggplot() + 
  geom_sf(data=md_beds_per_100k, aes(fill=beds_per_100k)) + 
  theme_minimal() +
